...
train_data_type1 = train_data[['ImageId','D4']][train_data['H4']==1]
...
t4_X_train = np.zeros((len(train_data_type1),256,1600,3),dtype=np.uint8)
t4_Y_train = np.zeros((len(train_data_type1),256,1600,1),dtype=np.uint8)
...
for idx, i in enumerate(train_data_type1.values,0):
    ...
    t4_X_train[idx] = temp_X
    t4_defect_ori.append((rle2mask(i[1]) == 1).sum())
    temp_Y = rle2mask(i[1])
    t4_Y_train[idx] = np.reshape(temp_Y,(256,1600,-1))
    t4_defect.append((t4_Y_train[idx] == 1).sum())
    '''plt.subplot(3,1,1)
...
'''for b in new_Y_train:
    t4_defect.append((b == 1).sum())

rmv_idx = []
for idx, item in enumerate(new_Y_train,0):
    if((((item == 1).sum())>np.percentile(t4_defect, 98)) or (((item == 1).sum())<np. percentile(t4_defect, 2))):
        rmv_idx.append(idx)'''

rmv_idx = []
'''for idx, item in enumerate(t4_Y_train,0):
    if((((item == 1).sum())>np.percentile(t4_defect, 98)) or (((item == 1).sum())<np. percentile(t4_defect, 2))):
        rmv_idx.append(idx)'''

t4_defect = []
#t4_X_train = np.delete(t4_X_train,rmv_idx,0)
#t4_Y_train = np.delete(t4_Y_train,rmv_idx,0)


new_X_train = np.zeros((len(t4_X_train)*4,256,384,3),dtype=np.uint8)
new_Y_train = np.zeros((len(t4_Y_train)*4,256,384,1),dtype=np.uint8)
rmv_idx = []
i = 0
print(len(new_X_train))
print(len(new_Y_train))
for a , b in zip(t4_X_train,t4_Y_train):
    temp_X = a[:,:400,:]
    ...
    if(np.all(new_Y_train[i]==0)):
        ...
        t4_defect.append((new_Y_train[i] == 1).sum())
    i = i + 1
    temp_X = a[:,400:800,:]
    temp_img = cv2.resize(temp_X,(384,256),interpolation = cv2.INTER_LINEAR)
    #temp_img = np.reshape(temp_img,(256,384,-1))
...
df1.to_csv('t4_final.csv',index=False)
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_acc_final.png')
plt.clf()
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_loss_final.png')
plt.clf()
'''def tversky(y_true, y_pred, smooth=1e-6, alpha=0.9):
    y_true_pos = K.flatten(y_true)
    y_pred_pos = K.flatten(y_pred)
...
plt.legend()
plt.title('ROC Curve')
plt.savefig('steel_t4_ROC_gamma.png')
plt.clf()'''
...
plt.legend()
plt.title('ROC Curve')
plt.savefig('steel_t4_ROC_lr.png')
plt.clf()
fig, ax = plt.subplots(1, 1, figsize=(10,6))
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_acc_lr.png')
plt.clf()
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_loss_lr.png')
plt.clf()
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_vacc_lr.png')
plt.clf()
...
ax.legend()
ax.grid()
fig.savefig('steel_t4_vloss_lr.png')
plt.clf()

DF = pd.concat([df1,df2,df3,df4],axis=0)
filename = 'steel_t4_report_lr.csv'
DF.to_csv(filename,index=False)

submission = pd.DataFrame({'LR': lr, 'DSC': DSC})
filename = 'steel_t4_DSC_lr.csv'
...
ax.set_ylabel(r'Loss', fontsize=20)
ax.legend()
fig.savefig('steel_t4_loss_whole.png')
plt.clf()
...
ax.legend()
fig.savefig('steel_t4_acc_whole.png')
plt.clf()'''
